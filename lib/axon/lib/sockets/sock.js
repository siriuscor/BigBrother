...
Socket.prototype.connect = function(origin_port, origin_host, fn){
  var self = this;
  if ('server' == this.type) throw new Error('cannot connect() after bind()');
  var port, host;
  if ('function' == typeof origin_host) {
    fn = origin_host;
    host = undefined;
  }
  
  if ('string' == typeof origin_port) {
    port = url.parse(origin_port);
  ...
  sock.on('close', function(){
    ...
    setTimeout(function(){
      ...
      sock.destroy();
      self.connect(origin_port, origin_host);
