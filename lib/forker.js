...
function start_id(id) {
    ...
    // if (running[id].logger) running[id].logger.close(); // clean last logger, UPDATE: cause write file end error when rotate file
    ...
    if (serviceConfig.stdout !== '/dev/null') {
        ...
        logger.add(new winston.transports.File(Object.assign({}, {
            level: 'info'
        }, default_log_setting, serviceConfig.stdout)));
    ...
    if (serviceConfig.stderr !== '/dev/null') {
        ...
        logger.add(new winston.transports.File(Object.assign({}, {
            level: 'error'
        }, default_log_setting, serviceConfig.stderr)));
